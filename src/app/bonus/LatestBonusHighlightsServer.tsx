...
type LatestBonusHighlightsServerProps = {
  page?: number;
  limit?: number;
  queryKey?: string;
  /** First 12 articles only; “View all” → /bonus/highlights */
  teaser?: boolean;
  /** Set false on /bonus/highlights when the hero already shows the title */
  showSectionHeader?: boolean;
};

export async function LatestBonusHighlightsServer({
  page = 1,
  limit = 12,
  queryKey = "page",
  teaser = false,
  showSectionHeader = true,
}: LatestBonusHighlightsServerProps = {}) {
  ...
  try {
    const res = await fetch(
      `${API_PUBLIC}/bonus-articles?page=${fetchPage}&limit=${limit}`,
      {
        cache: "no-store",
      },
    );
    ...
    >
      <Box sx={{ flex: "1 1 240px", minWidth: 0 }}>
        ...
        <Typography
          ...
          sx={{ maxWidth: 560 }}
        >
          Stay updated with the latest bonus launches, limited-time promotions,
          and exclusive offers
        </Typography>
      </Box>
      <Link
        href="/bonus/highlights"
      ...
      <Box sx={{ maxWidth: 1280, mx: "auto" }}>
        ...
        >
          {items.length ? (
            ...
            >
              No bonus highlights found right now.
            </Typography>
          )}
        </Box>
        {!teaser ? (
