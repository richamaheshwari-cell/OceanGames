...
import { ArticleGamesSection } from "./ArticleGamesSection";
...
type ArticleResponse = {
  ...
  readTime?: string | null;
  editor?: Editor | null;
  relatedArticleCasino?: RelatedItem[];
...
async function getArticle(slug: string): Promise<ArticleResponse | null> {
  ...
  const res = await fetch(`${base}/api/v1/public/casino-articles/${slug}`, {
...
function SidebarItem({
  ...
  item: RelatedItem;
  basePath: "casino-articles" | "game-articles";
  isDark: boolean;
}) {
...
export default async function CasinoArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const relatedCasino = article.relatedArticleCasino ?? [];
  ...
  const path = `/casino-articles/${article.slug}`;
  ...
  return (
    <>
      ...
      {/* Hero: feature image as background */}
      <Box
        sx={{
          position: "relative",
      ...
      <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, sm: 3 }, py: 2 }}>
        ...
        >
          ...
          >
            {/* Trending Now = relatedArticleCasino */}
            {relatedCasino.length > 0 && (
              <Box
                sx={{
                  bgcolor: "grey.900",
                  color: "white",
              ...
              >
                ...
                {relatedCasino.map((item) => (
                  <SidebarItem
                    key={item.id}
                    item={item}
                    basePath="casino-articles"
                    isDark
                  />
                ))}
              </Box>
            ...
            {/* LatestGameUpdates = relatedArticleGames */}
